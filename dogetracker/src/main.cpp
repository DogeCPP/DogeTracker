...
static Tracker* gTracker = nullptr;
...
PLUGIN_API int XPluginStart(char * name, char * sig, char * desc)
{
    ...
    strcpy(desc, "Real-time flight tracker with AviTab integration");
    ...
    #if DEBUG
    Dataref::getInstance()->createCommand("dogetracker/debug/window_to_foreground", "Bring window to front", [](XPLMCommandPhase inPhase) {
    ...
    Dataref::getInstance()->createCommand("dogetracker/debug/vr_click_proxy", "sim/VR/reserved/select", [](XPLMCommandPhase inPhase) {
...
void menuAction(void* mRef, void* iRef) {
    if (!strcmp((char *)iRef, "ActionOpenMap")) {
        // Open the DogeTracker live map inside the AviTab browser
        if (AppState::getInstance()->browser) {
            AppState::getInstance()->showBrowser(DOGETRACKER_API_URL "/map");
        }
    }
        ...
        params.drawWindowFunc = [](XPLMWindowID inWindowID, void *drawingRef){
            ...
            y -= 32.0f;
            XPLMDrawString(color, x, y, "Real-time flight tracker with AviTab integration for X-Plane.", nullptr, xplmFont_Proportional);
            y -= 32.0f;
            ...
            y -= 16.0f;
            XPLMDrawString(color, x, y, "For updates, see github.com/yourusername/dogetracker.", nullptr, xplmFont_Proportional);
            y -= 16.0f;
            XPLMDrawString(color, x, y, "Based on avitab-browser by TheRamon. Fly safe!", nullptr, xplmFont_Proportional);
        };
...
float update(float inElapsedSinceLastCall, float inElapsedTimeSinceLastFlightLoop, int inCounter, void *inRefcon) {
    ...
    // Broadcast position to DogeTracker server
    if (gTracker) {
        gTracker->tick();
    }
